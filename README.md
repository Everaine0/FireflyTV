...
文本），任何一个写点写错都会把全部记录写坏 —— 用户报的「多次切换以后记录丢了」就是这么来的。
...
.\build.ps1 testDebugUnitTest    # 单元测试（228 项：农历/节气/缓存/导航/按键反馈/格式判定/解码策略/渲染通路/moov 改写/数据源约定/观看记录/天气策略/界面缩放/选码与 10bit 防线/帧率归因/媒体库结构）
...
> ⚠️ 这台机器是「WSL 里编辑、Windows 里构建」两头一套源码。用 `scripts/win-build.sh`
> 构建，Windows 侧那棵树的位置用 `WIN_ROOT=` 传进去（路径因机器而异，不写进仓库）——
> 它会连 `app/build.gradle.kts`、`proguard-rules.pro`、ijkplayer AAR 一起同步过去。
> 以前只同步 `app/src`，于是「改了构建脚本却不生效」，白忙过一轮。
