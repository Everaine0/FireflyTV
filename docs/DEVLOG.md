...
- [起播慢就说「无法播放」，一集播完直接黑屏](#起播慢就说无法播放一集播完直接黑屏)
- [播放记录](#播放记录)
- [天气](#天气)
- [诊断页与实验台](#诊断页与实验台)
- [实测记录](#实测记录)
...
这一轮的两个问题都不是「放不了」，而是**界面说错了话**：一个把"慢"当成"坏"，
一个把点播的一集播完当成直播断流。

### 现象 1：偶尔打开显示「这个视频无法播放」，等一段时间又正常

用户的原话，连带他自己的判断：

> 偶尔打开会显示这个视频无法播放，等一段时间正常。推测是由于 nas 的硬盘休眠机制导致的
> 首次启动较慢，但不应该立刻显示无法播放呀，可以考虑添加一个正在加载中，
> 超过某个时间你再显示这个。

**判断是对的，链路也确认了：**

1. SMB 的每一次请求超时是 **20 秒**（`SmbClient` 的 `SmbConfig.withTimeout(20, SECONDS)`；
   那个类的注释里解释了为什么不能用 `withSoTimeout` —— smbj 会把它当成致命传输错误，
   直接关掉整条连接）；
2. 硬盘休眠时，播放器的第一次读正好撞在这个超时上 → `TimeoutException`；
3. 异常穿过 `SmbMediaDataSource.readAt` → JNI → ijkplayer 把它报成一次播放错误（`onError`）；
4. 引擎把播放错误翻成「这个视频无法播放」，界面层**立刻**弹故障页；
5. 而十几秒后硬盘转起来了，同一集其实能播。

也就是说：**片源没问题、NAS 也没问题，错的是那句过早的结论。**

### 修法：没出画面之前，失败先算「还在加载」

- 起播（`playEpisodeNow` / `playChannelNow`）先挂一条 **「正在加载…」**（`SwitchHud.onStarting`），
  首帧上屏才换成内容名 —— 这段时间原来屏幕上**一个字都没有**，老人分不清"在加载"和"坏了"；
- 没出过画面时的失败**不再立刻报故障**，而是自动重试：判据抽成纯函数 `StartupGrace`
  （窗口 40 秒、最多 3 次、退避 3/8/15 秒），超过窗口才把原因摆到屏幕上；
- 已经在放的片子中途断了不走这条路（`gotFirstFrame` 已经是 true）：那种情况用户需要**立刻**知道原因。

为什么窗口取 40 秒：SMB 读超时是 20 秒一刀，窗口至少要容得下「超时一次 + 再试一次」；
再长就不像话了 —— 真打不开的片子让老人对着黑屏等一分多钟，比看一句人话更糟。

顺手还堵掉一个会在同一场景下发作的竞态：`startPlayback` 里打开字节源那一段是**阻塞**的
（硬盘休眠时一次请求等满 20 秒超时，`SmbStore` 自己还会再试一次，最长约 40 秒），
而界面层的起播看门狗 25 秒就会到点、调 `retryInSoftware()` 让引擎**在另一条线程上**
再起一次 —— 于是两个播放器同时挂在同一个 Surface 上，界面层却只跟其中一个。
现在引擎用 `starting` 把「正在打开字节源」标出来，这段时间不让软解重播插队
（瓶颈在 NAS 读取，退回软解也救不了）：让这一次走完，失败由起播宽限接着重试。

> 这条宽限还顺手封了一条不显眼的死路：`onCompletion` 里「时长不可信 → 不跳集」
> 原来只有一行日志就 `return`，而此刻播放器已经停在文件结尾、屏幕全黑，
> 起播看门狗早在这一集开头就撤了 —— 永远不会自己好。现在它和别的故障一样，
> 留一句人话 + 10 秒重试。

### 现象 2：上一集播完，下一集直接黑屏（不是原因未知，是状态串了）

用户报的是「铁梨花播放偶尔会出现上一集播完，下一集直接黑屏。原因未知」。

**根因：`liveReconnect` 这个开关从来没有被关掉过。**

引擎原来只用一个布尔量决定「播完了」该怎么办：

```kotlin
p.setOnCompletionListener {
    if (liveReconnect) scheduleLiveRetry() else onMain { listener?.onCompletion() }
}
```

而 `liveReconnect` 是界面层**换到直播时**打开的（`playChannelNow` 里的
`setLiveReconnect(true)`），全工程**没有任何一处把它置回 false**。于是
「先看直播、再回电视剧」以后：

1. 点播的一集播完 → 引擎看到 `liveReconnect == true` → 判定「这是直播断流」；
2. `scheduleLiveRetry()` 第一件事是 `releaseInternal()` —— 播放器被释放，**画面当场变黑**；
3. 两秒后 `retryRunnable` 去取频道地址重连，而地址来自界面层的
   `channels.getOrNull(channelIndex)?.url` —— 切库时 `channels` 已经被清空了
   （`selectLibrary` 的「换库即作废上一个库的所有缓存」），provider 返回 null；
4. `retryRunnable` 直接 `return` —— **黑屏就永远停在那儿**。

没有报错、也没有看门狗（起播看门狗在上一集首帧时就撤了），所以现场看起来就是
「黑屏、按键还有反应」。这也解释了为什么是「偶尔」：同一个会话里没看过直播就不会踩到。

**修法**（两处一起改，缺一处都能再踩）：

- 判据加另一半：`PlaybackMode.onCompletion(kind, liveReconnect)` —— 只有「正在播直播」
  而且开着重连时，「播完」才等于断流。抽成纯函数 + 单测（`PlaybackModeTest`），
  因为这条规则错了**不会报错，只会黑屏**；
- 引擎切到点播内容时（`playSmb` / `playSource`）自己 `stopLiveReconnect()`：状态跟着内容
  类型走，界面层漏一处就是一次黑屏。顺带把 provider 一起清掉 —— 万一有一个直播重连的
  Runnable 已经排在主线程队列里（切换过程中很常见），它醒来拿不到地址就不会去抢画面。

顺手还封了一条同类死路：`advanceToNextShow` 拿到 IO 线程现扫出来的剧列表后没有回写
`shows` 字段，而 `playShow` 是按那个字段找剧的 —— 字段为空时它第一行就 `return`，
同一条黑屏。
...
| 项 | 内容 |
| :--- | :--- |
| `StartupGraceTest`（6 项） | 宽限窗口、重试次数、「出过画面就不再宽限」、退避递增且有上限，以及「窗口必须大于 20 秒的读超时」 |
| `PlaybackModeTest`（+2 项） | 点播播完必须跳下一集（哪怕直播开关还开着）；只有直播才把「播完」当断流 |
| `SwitchHudTest`（+3 项） | 「正在加载…」立刻显示、首帧之前不许消失、提示过期后首帧才到也要报出名字 |
| 全量单测 | **259 项**全绿（原 248 项） |

> 复现依赖真实环境（NAS 硬盘休眠 / 先看直播再回点播），模拟器上造不出来，
> 所以这一轮的结论只到「源码核对 + 单测钉住」这一步；真机上的实际表现要看下一轮反馈。
...
| 单元测试 / 插桩测试 | **195 / 58，全绿**（当次快照；这两个数字随测试增加，最新值以 README 的「构建」一节为准） |
