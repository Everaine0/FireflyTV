...
>
> 定稿 2026-09-11 ｜ 状态：阶段 1–4 已落地，模拟器全流程验证；真机待复测（见 §9 风险 21–23 与「待复测」表）
...
| 实机排错 | 长按 OK / 「信息」键 → 播放诊断页（解码通路真值 + 帧率 + 读取 + 结论） |
...
- **库列表刷新后按下标前必须按名字重新锚定**：缓存里的库列表可能比 NAS 上少一个
  （用户新加文件夹就是这样），刷新回来时旧下标指向的已经是**另一个库** ——
...
| 14 | **电视是否真能硬解 4K H.265 未知**（HEVC 的 UHD 档在 CDD 里只是 SHOULD，且 ijkplayer 不查 profile） | ⏳ **必须上真机跑 `CodecCapabilityProbeTest`**，见风险 14 |
...
1. 上真机先跑 `CodecCapabilityProbeTest` —— 打印每个 `video/hevc` 解码器的
...
**用户反馈**：「除直播外的其他影视库没有做播放记录功能 —— 无法做到启动时自动切换到
...
**两条「宁可少写，也不写坏」的规矩**（用户后续反馈的「多次切换或者其他原因会导致播放记录
...
> 电视上同样是纯软解 —— 用户后来真机复现的「极慢、还没声音」就是它（见「风险 9」）。
...
**现象（用户原话）**：「感觉电视上按下 OK 大概就中心那一块有内容，应该是没有缩放」。
...
**排查入口**：长按 OK（或遥控器「信息」键）打开诊断页，「解码」那一行直接写着
...
| 解码 | `硬解 OMX.MTK.VIDEO.DECODER.HEVC` / `软解（硬解没建成！）` | 用户那句「硬解好像没开」的直接答案 |
...
用户那句「帧率不高」还有一个候选根因是「读数据供不上」。这台 NAS 上实测（模拟器直连，
...
**现象（用户实机实测，诊断页读数）**：《大宅门》（3840×2160 HEVC 25 帧）明显不够流畅，
而同一台电视上《猫和老鼠》（2960×2160 HEVC 23.976 帧）、《娘道》（1080p50 H.264）、
用户新加的测试片（**3840×2160 H.264 50 帧**，中位 48.5 帧）都正常。
...
所以应用在 **debug 包里**开了一个局域网 HTTP 口（`DiagHub`，端口 **8642**，无鉴权 —— 用户要求
「怎么简单怎么来」，里面也不经手任何凭据）：
...
用户说电视上「不能/不方便」连 adb，所以这两条要挑能做的时候做；
...
| **7** | **真机复现「极慢 + 没声音」→ 打开 MediaCodec 硬解 + 软解兜底** | ✅ 已改完（风险 9），**待真机复测** |
...
| **电视上 4K 的帧率还是不够** | ⏳ **待真机复测（已给出屏幕上的证据入口）** | 用户实机反馈：「还算流畅、声音也跟得上，但明显感觉帧率不高，25 甚至更低」。三种根因（解码不够快 / 送显来不及 / 读取供不上）在电视上没法用 adb 区分，所以加了**播放诊断页**：**长按 OK 或按「信息」键**，看「解码」「帧率」「结论」三行 —— `硬解 OMX.xxx` + `解码 ≈ 片源帧率` 说明通路没问题，`软解（硬解没建成！）` 就是风险 22。复测时同时抓 `adb logcat -s FireflyDecode FireflyTV`（如果方便接 adb） |
| 硬解真值 | ⏳ 待真机 | 模拟器上只有 `OMX.google.*`（`RANK_SOFTWARE=200`，被 ijkplayer 拒掉），所以**「选中硬解 → 真的建成」这条路在模拟器上跑不到**。真机要看诊断页的「解码」行或日志 `解码通路确认：硬解 OMX.xxx（vdec_type=2）`。若出现 `硬解没建成：选中的 … 实际没接上`，说明这台电视的 MediaCodec 建得出来却接不上，代码会自动拉黑它换下一个候选 |
